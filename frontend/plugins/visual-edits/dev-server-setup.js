      ...
      try {
        ...
        Object.entries(changesByFile).forEach(([fileName, fileChanges]) => {
          ...
          try {
            ...
            execSync(`git -c user.name="kapta-edit" -c user.email="contact@kaptamedia.fr" add "${targetFile}"`);
            execSync(`git -c user.name="kapta-edit" -c user.email="contact@kaptamedia.fr" commit -m "visual_edit_${timestamp}"`);
          } catch (gitError) {
